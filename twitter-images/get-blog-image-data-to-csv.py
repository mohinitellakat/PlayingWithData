...
cur.execute("""SELECT a.gender, a.state, b.profile_url, c.blog_url, d.post_url, d.img_url
                FROM profiles AS a 
                LEFT OUTER JOIN profiles_blogs AS b ON a.url = b.profile_url
                LEFT OUTER JOIN blogs_posts AS c ON b.blog_url = c.blog_url
                LEFT OUTER JOIN posts_images AS d ON c.post_url = d.post_url
...
c = csv.writer(open("C:\\Users\\mt34546\\Documents\\UTStuff\\PennebakerLabStuff\\UMich-Summer2017\\img_gender_state.csv","wb"))
for row in dataset:
    c.writerow(row)
